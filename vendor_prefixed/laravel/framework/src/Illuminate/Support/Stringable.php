...
class Stringable implements JsonSerializable
{
    ...
    {
        return new static(XT_class_basename($this->value));
    }
    ...
    {
        return XT_collect(explode($delimiter, $this->value, $limit));
    }
    ...
    public function split($pattern, $limit = -1, $flags = 0)
    {
        if (filter_var($pattern, FILTER_VALIDATE_INT) !== false) {
            return XT_collect(mb_str_split($this->value, $pattern));
        }

        $segments = preg_split($pattern, $this->value, $limit, $flags);

        return ! empty($segments) ? XT_collect($segments) : XT_collect();
    }
