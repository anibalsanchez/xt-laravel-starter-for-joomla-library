...
namespace Extly\Illuminate\Support\Testing\Fakes;

use Closure;
use Extly\Illuminate\Bus\PendingBatch;
use Extly\Illuminate\Contracts\Bus\QueueingDispatcher;
...
class BusFake implements QueueingDispatcher
{
    ...
    public function assertChained(array $expectedChain)
    {
        ...
        PHPUnit::assertTrue(
            XT_collect($expectedChain)->isNotEmpty(),
    ...
    {
        return XT_tap(clone $job, function ($job) {
    ...
    {
        $chain = XT_collect($expectedChain)->map(function ($job) {
    ...
    protected function assertDispatchedWithChainOfClasses($command, $expectedChain, $callback)
    {
        $matching = $this->dispatched($command, $callback)->map->chained->map(function ($chain) {
            return XT_collect($chain)->map(function ($job) {
    ...
    {
        return ! XT_collect($chain)->contains(function ($job) {
    ...
    public function dispatched($command, $callback = null)
    {
        ...
        return XT_collect($this->commands[$command])->filter(function ($command) use ($callback) {
    ...
    public function dispatchedSync(string $command, $callback = null)
    {
        ...
        return XT_collect($this->commandsSync[$command])->filter(function ($command) use ($callback) {
            return $callback($command);
        });
    }
    ...
    public function dispatchedAfterResponse(string $command, $callback = null)
    {
        ...
        return XT_collect($this->commandsAfterResponse[$command])->filter(function ($command) use ($callback) {
            return $callback($command);
        });
    }
    ...
    public function batched(callable $callback)
    {
        ...
        return XT_collect($this->batches)->filter(function ($batch) use ($callback) {
    ...
    protected function shouldFakeJob($command)
    {
        ...
        return XT_collect($this->jobsToFake)
