...
use Closure;
use Extly\Illuminate\Contracts\Mail\Factory;
use Extly\Illuminate\Contracts\Mail\Mailable;
use Extly\Illuminate\Contracts\Mail\Mailer;
use Extly\Illuminate\Contracts\Mail\MailQueue;
...
class MailFake implements Factory, Mailer, MailQueue
{
    ...
    {
        $mailableNames = XT_collect($this->mailables)->map(function ($mailable) {
    ...
    {
        $mailableNames = XT_collect($this->queuedMailables)->map(function ($mailable) {
            return get_class($mailable);
        })->join(', ');
    ...
    {
        return XT_collect($this->mailables)->filter(function ($mailable) use ($type) {
    ...
    {
        return XT_collect($this->queuedMailables)->filter(function ($mailable) use ($type) {
            return $mailable instanceof $type;
        });
    }
