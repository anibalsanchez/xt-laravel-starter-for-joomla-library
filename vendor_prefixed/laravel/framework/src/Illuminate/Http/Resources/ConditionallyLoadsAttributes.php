...
trait ConditionallyLoadsAttributes
{
    ...
    {
        if ($condition) {
            return XT_value($value);
        }

        return func_num_args() === 3 ? XT_value($default) : new MissingValue;
    }
    ...
    {
        return $condition ? new MergeValue(XT_value($value)) : new MissingValue;
    }
    ...
    protected function whenAppended($attribute, $value = null, $default = null)
    {
        if ($this->resource->hasAppended($attribute)) {
            return func_num_args() >= 2 ? XT_value($value) : $this->resource->$attribute;
        }
    ...
    {
        return XT_transform($value, $callback, func_num_args() === 3 ? $default : new MissingValue
        );
    }
}
