...
class PendingRequest
{
    ...
    {
        return XT_tap($this, function ($request) use ($format) {
    ...
    {
        return XT_tap($this, function ($request) use ($headers) {
    ...
    {
        return XT_tap($this, function ($request) use ($username, $password) {
    ...
    {
        return XT_tap($this, function ($request) use ($token, $type) {
    ...
    {
        return XT_tap($this, function ($request) use ($userAgent) {
    ...
    {
        return XT_tap($this, function ($request) use ($cookies, $domain) {
            return $this->options = array_merge_recursive($this->options, [
    ...
    {
        return XT_tap($this, function ($request) {
    ...
    {
        return XT_tap($this, function ($request) use ($to) {
    ...
    {
        return XT_tap($this, function () use ($seconds) {
    ...
    {
        return XT_tap($this, function ($request) use ($options) {
    ...
    {
        return XT_tap($this, function () use ($callback) {
    ...
    public function pool(callable $callback)
    {
        ...
        $requests = XT_tap(new Pool($this->factory), $callback)->getRequests();
    ...
    public function send(string $method, string $url, array $options = [])
    {
        ...
        return XT_retry($this->tries ?? 1, function () use ($method, $url, $options) {
            try {
                return XT_tap(new Response($this->sendRequest($method, $url, $options)), function ($response) {
    ...
    {
        return XT_collect($data)->map(function ($value, $key) {
    ...
    {
        $clientMethod = $this->async ? 'requestAsync' : 'XT_request';
    ...
    {
        return XT_tap($handlerStack, function ($stack) {
        ...
        return function ($handler) {
            return function ($request, $options) use ($handler) {
                ...
                return $promise->then(function ($response) use ($request, $options) {
                    XT_optional($this->factory)->recordRequestResponsePair(
    ...
    public function buildStubHandler()
    {
        return function ($handler) {
            return function ($request, $options) use ($handler) {
                $response = ($this->stubCallbacks ?? XT_collect())
                     ->map
    ...
    {
        $this->stubCallbacks = XT_collect($callback);
