...
use Extly\Symfony\Component\HttpFoundation\HeaderBag;
use Throwable;
...
trait ResponseTrait
{
    ...
    {
        if (is_string($cookie) && function_exists('XT_cookie')) {
            $cookie = XT_cookie(...func_get_args());
        }
    ...
    {
        if (is_string($cookie) && function_exists('XT_cookie')) {
            $cookie = XT_cookie($cookie, null, -2628000, $path, $domain);
        }
