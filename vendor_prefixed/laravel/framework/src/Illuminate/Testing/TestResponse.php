...
EOF;
    ...
    {
        PHPUnit::assertEquals(
            XT_app('url')->to($uri), XT_app('url')->to($this->headers->get('Location'))
        );
    ...
    public function assertCookie($cookieName, $value = null, $encrypted = true, $unserialize = false)
    {
        ...
        $actual = $encrypted
            ? CookieValuePrefix::remove(XT_app('encrypter')->decrypt($cookieValue, $unserialize))
            : $cookieValue;
    ...
    public function assertSeeText($value, $escape = true)
    {
        ...
        XT_tap(strip_tags($this->getContent()), function ($content) use ($values) {
            foreach ($values as $value) {
    ...
    public function dumpSession($keys = [])
    {
        ...
        if (empty($keys)) {
            XT_dump($this->session()->all());
        } else {
            XT_dump($this->session()->only($keys));
        }
