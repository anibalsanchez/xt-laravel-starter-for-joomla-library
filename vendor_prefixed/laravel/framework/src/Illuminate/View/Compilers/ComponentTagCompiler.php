...
use Extly\Illuminate\Contracts\View\Factory;
use Extly\Illuminate\Filesystem\Filesystem;
use Extly\Illuminate\Support\Str;
use Extly\Illuminate\View\AnonymousComponent;
use Extly\Illuminate\View\DynamicComponent;
use Extly\Illuminate\View\ViewFinderInterface;
...
<?php $component->withAttributes(['.$this->attributesToString($attributes->all(), $escapeAttributes = $class !== DynamicComponent::class).']); ?>';
    ...
    public function partitionDataAndAttributes($class, array $attributes)
    {
        ...
        if (! class_exists($class)) {
            return [XT_collect($attributes), XT_collect($attributes)];
        }

        $constructor = (new ReflectionClass($class))->getConstructor();

        $parameterNames = $constructor
                    ? XT_collect($constructor->getParameters())->map->getName()->all()
                    : [];

        return XT_collect($attributes)->partition(function ($value, $key) use ($parameterNames) {
    ...
    protected function getAttributesFromAttributeString(string $attributeString)
    {
        ...
        return XT_collect($matches)->mapWithKeys(function ($match) {
    ...
    {
        return XT_collect(token_get_all($value))->map(function ($token) {
            if (! is_array($token)) {
                return $token;
            }
