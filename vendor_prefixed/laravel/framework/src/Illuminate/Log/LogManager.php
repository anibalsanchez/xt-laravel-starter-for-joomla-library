...
use Extly\Monolog\Handler\ErrorLogHandler;
use Extly\Monolog\Handler\FormattableHandlerInterface;
use Extly\Monolog\Handler\HandlerInterface;
use Extly\Monolog\Handler\RotatingFileHandler;
use Extly\Monolog\Handler\SlackWebhookHandler;
use Extly\Monolog\Handler\StreamHandler;
...
class LogManager implements LoggerInterface
{
    ...
    {
        try {
            return $this->channels[$name] ?? XT_with($this->resolve($name), function ($logger) use ($name) {
                return $this->channels[$name] = $this->tap($name, new Logger($logger, $this->app['events']));
            });
        } catch (Throwable $e) {
            return XT_tap($this->createEmergencyLogger(), function ($logger) use ($e) {
    ...
    protected function createStackDriver(array $config)
    {
        ...
        $handlers = XT_collect($config['channels'])->flatMap(function ($channel) {
            return $this->channel($channel)->getHandlers();
        })->all();

        $processors = XT_collect($config['channels'])->flatMap(function ($channel) {
    ...
    {
        return XT_tap(new LineFormatter(null, $this->dateFormat, true, true), function ($formatter) {
