...
use Extly\Illuminate\Contracts\Broadcasting\ShouldBroadcast;
use Extly\Illuminate\Contracts\Container\Container as ContainerContract;
use Extly\Illuminate\Contracts\Events\Dispatcher as DispatcherContract;
use Extly\Illuminate\Contracts\Queue\ShouldBeEncrypted;
...
class Dispatcher implements DispatcherContract
{
    ...
    public function listen($events, $listener = null)
    {
        if ($events instanceof Closure) {
            return XT_collect($this->firstClosureParameterTypes($events))
                ->each(function ($event) use ($events) {
                    $this->listen($event, $events);
                });
        } elseif ($events instanceof QueuedClosure) {
            return XT_collect($this->firstClosureParameterTypes($events->closure))
                ->each(function ($event) use ($events) {
    ...
    {
        return XT_tap($job, function ($job) use ($listener) {
