...
namespace Extly\Illuminate\Mail;

use Extly\Illuminate\Container\Container;
use Extly\Illuminate\Contracts\Filesystem\Factory as FilesystemFactory;
use Extly\Illuminate\Contracts\Mail\Factory as MailFactory;
use Extly\Illuminate\Contracts\Mail\Mailable as MailableContract;
use Extly\Illuminate\Contracts\Queue\Factory as Queue;
...
class Mailable implements MailableContract, Renderable
{
    ...
    protected function buildSubject($message)
    {
        if ($this->subject) {
            ...
            $message->subject(Str::title(Str::snake(XT_class_basename($this), ' ')));
        }
    ...
    protected function hasRecipient($address, $name = null, $property = 'to')
    {
        ...
        return XT_collect($this->{$property})->contains(function ($actual) use ($expected) {
    ...
    {
        $this->attachments = XT_collect($this->attachments)
    ...
    {
        $this->diskAttachments = XT_collect($this->diskAttachments)->push([
            'disk' => $disk,
            'path' => $path,
    ...
    {
        $this->rawAttachments = XT_collect($this->rawAttachments)
