...
use Exception;
use Extly\Illuminate\Bus\Batchable;
...
use Extly\Illuminate\Contracts\Encryption\Encrypter;
use Extly\Illuminate\Contracts\Queue\Job;
use Extly\Illuminate\Contracts\Queue\ShouldBeUnique;
use Extly\Illuminate\Contracts\Queue\ShouldBeUniqueUntilProcessing;
use Extly\Illuminate\Database\Eloquent\ModelNotFoundException;
use Extly\Illuminate\Pipeline\Pipeline;
...
class CallQueuedHandler
{
    ...
    {
        if (in_array(InteractsWithQueue::class, XT_class_uses_recursive($instance))) {
            $instance->setJob($job);
        }
    ...
    {
        $uses = XT_class_uses_recursive($command);
    ...
    {
        if (! in_array(Batchable::class, XT_class_uses_recursive($command)) ||
            is_null($command->batch())) {
            return;
        }
