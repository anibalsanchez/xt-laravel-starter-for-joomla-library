...
use Extly\Illuminate\Queue\Events\JobQueued;
...
abstract class Queue
{
    ...
    protected function createObjectPayload($job, $queue)
    {
        $payload = $this->withCreatePayloadHooks($queue, [
            ...
            'job' => 'Extly\Illuminate\Queue\CallQueuedHandler@call',
    ...
    public function getJobBackoff($job)
    {
        ...
        return XT_collect(Arr::wrap($job->backoff ?? $job->backoff()))
        ...
        if ($this->shouldDispatchAfterCommit($job) &&
            ...
            return $this->container->make('db.transactions')->addCallback(
                function () use ($payload, $queue, $delay, $callback, $job) {
                    return XT_tap($callback($payload, $queue, $delay), function ($jobId) use ($job) {
