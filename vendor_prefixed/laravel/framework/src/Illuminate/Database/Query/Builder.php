...
namespace Extly\Illuminate\Database\Query;

use Closure;
use DateTimeInterface;
use Extly\Illuminate\Contracts\Support\Arrayable;
use Extly\Illuminate\Database\Concerns\BuildsQueries;
use Extly\Illuminate\Database\Concerns\ExplainsQueries;
use Extly\Illuminate\Database\ConnectionInterface;
use Extly\Illuminate\Database\Eloquent\Builder as EloquentBuilder;
use Extly\Illuminate\Database\Eloquent\Relations\Relation;
use Extly\Illuminate\Database\Query\Grammars\Grammar;
use Extly\Illuminate\Database\Query\Processors\Processor;
use Extly\Illuminate\Pagination\CursorPaginationException;
use Extly\Illuminate\Pagination\CursorPaginator;
...
class Builder
{
    ...
    {
        return XT_collect($this->onceWithColumns(Arr::wrap($columns), function () {
    ...
    public function paginate($perPage = 15, $columns = ['*'], $pageName = 'page', $page = null)
    {
        ...
        $results = $total ? $this->forPage($page, $perPage)->get($columns) : XT_collect();
    ...
    protected function ensureOrderForCursorPagination($shouldReverse = false)
    {
        ...
        $orderDirections = XT_collect($this->orders)->pluck('direction')->unique();
        ...
        if ($shouldReverse) {
            $this->orders = XT_collect($this->orders)->map(function ($order) {
    ...
    protected function stripTableForPluck($column)
    {
        ...
        return XT_last(preg_split('~'.$separator.'~i', $column));
    }
    ...
    public function upsert(array $values, $uniqueBy, $update = null)
    {
        ...
        $bindings = $this->cleanBindings(array_merge(
            ...
            XT_collect($update)->reject(function ($value, $key) {
                return is_int($key);
            })->all()
        ));
    ...
    {
        return is_array($value) ? XT_head(Arr::flatten($value)) : $value;
    }
    ...
    {
        return XT_tap($this->clone(), function ($clone) use ($properties) {
    ...
    {
        return XT_tap($this->clone(), function ($clone) use ($except) {
    ...
    {
        XT_dump($this->toSql(), $this->getBindings());
