...
class SQLiteGrammar extends Grammar
{
    ...
    protected function compileUpdateColumns(Builder $query, array $values)
    {
        ...
        return XT_collect($values)->reject(function ($value, $key) {
            return $this->isJsonSelector($key);
        })->merge($jsonGroups)->map(function ($value, $key) use ($jsonGroups) {
            $column = XT_last(explode('.', $key));
    ...
    public function compileUpsert(Builder $query, array $values, array $uniqueBy, array $update)
    {
        ...
        $columns = XT_collect($update)->map(function ($value, $key) {
    ...
    protected function compileUpdateWithJoinsOrLimit(Builder $query, array $values)
    {
        ...
        $alias = XT_last(preg_split('/\s+as\s+/i', $query->from));
    ...
    public function prepareBindingsForUpdate(array $bindings, array $values)
    {
        ...
        $values = XT_collect($values)->reject(function ($value, $key) {
            return $this->isJsonSelector($key);
