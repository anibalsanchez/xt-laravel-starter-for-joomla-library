...
namespace Extly\Illuminate\Database\Query\Grammars;

use Extly\Illuminate\Database\Grammar as BaseGrammar;
use Extly\Illuminate\Database\Query\Builder;
use Extly\Illuminate\Database\Query\JoinClause;
...
class Grammar extends BaseGrammar
{
    ...
    {
        return XT_collect($joins)->map(function ($join) use ($query) {
    ...
    {
        return XT_collect($query->wheres)->map(function ($where) use ($query) {
    ...
    protected function compileHavingBetween($having)
    {
        ...
        $min = $this->parameter(XT_head($having['values']));

        $max = $this->parameter(XT_last($having['values']));
    ...
    public function compileInsert(Builder $query, array $values)
    {
        ...
        $parameters = XT_collect($values)->map(function ($record) {
    ...
    {
        return XT_collect($values)->map(function ($value, $key) {
    ...
    {
        $alias = XT_last(explode(' as ', $table));
