...
use Extly\Illuminate\Contracts\Queue\QueueableCollection;
use Extly\Illuminate\Contracts\Queue\QueueableEntity;
...
use Extly\Illuminate\Database\Eloquent\Collection as EloquentCollection;
use Extly\Illuminate\Database\Eloquent\Relations\BelongsToMany;
use Extly\Illuminate\Database\Eloquent\Relations\Concerns\AsPivot;
use Extly\Illuminate\Database\Eloquent\Relations\HasManyThrough;
use Extly\Illuminate\Database\Eloquent\Relations\Pivot;
use Extly\Illuminate\Support\Arr;
use Extly\Illuminate\Support\Collection as BaseCollection;
...
abstract class Model implements Arrayable, ArrayAccess, HasBroadcastChannel, Jsonable, JsonSerializable, QueueableEntity, UrlRoutable
{
    ...
    protected static function bootTraits()
    {
        ...
        foreach (XT_class_uses_recursive($class) as $trait) {
            $method = 'boot'.XT_class_basename($trait);
            ...
            if (method_exists($class, $method = 'initialize'.XT_class_basename($trait))) {
    ...
    protected function incrementOrDecrement($column, $amount, $extra, $method)
    {
        ...
        return XT_tap($this->setKeysForSaveQuery($query)->{$method}($column, $amount, $extra), function () use ($column) {
            $this->syncChanges();
    ...
    public function refresh()
    {
        ...
        $this->load(XT_collect($this->relations)->reject(function ($relation) {
            return $relation instanceof Pivot
                || (is_object($relation) && in_array(AsPivot::class, XT_class_uses_recursive($relation), true));
        })->keys()->all());
    ...
    public function replicate(array $except = null)
    {
        ...
        return XT_tap(new static, function ($instance) use ($attributes) {
    ...
    {
        return $this->table ?? Str::snake(Str::pluralStudly(XT_class_basename($this)));
    }
    ...
    {
        return Str::snake(XT_class_basename($this)).'_'.$this->getKeyName();
    }
    ...
    {
        return str_replace('\\', '.', get_class($this)).'.{'.Str::camel(XT_class_basename($this)).'}';
    }
