...
use Extly\Illuminate\Database\Query\Builder as QueryBuilder;
...
class Builder
{
    ...
    public function firstOrCreate(array $attributes = [], array $values = [])
    {
        ...
        return XT_tap($this->newModelInstance($attributes + $values), function ($instance) {
            $instance->save();
        });
    }
    ...
    {
        return XT_tap($this->firstOrNew($attributes), function ($instance) use ($values) {
    ...
    {
        $orderDirections = XT_collect($this->query->orders)->pluck('direction')->unique();
        ...
        if ($shouldReverse) {
            $this->query->orders = XT_collect($this->query->orders)->map(function ($order) {
        ...
        return XT_collect($this->query->orders);
    }
    ...
    {
        return XT_tap($this->newModelInstance($attributes), function ($instance) {
            $instance->save();
        });
    }
    ...
    {
        $whereBooleans = XT_collect($whereSlice)->pluck('boolean');
