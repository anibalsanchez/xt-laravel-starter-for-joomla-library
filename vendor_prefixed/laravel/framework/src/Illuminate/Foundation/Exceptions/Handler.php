...
use Extly\Illuminate\Contracts\Container\BindingResolutionException;
use Extly\Illuminate\Contracts\Container\Container;
use Extly\Illuminate\Contracts\Debug\ExceptionHandler as ExceptionHandlerContract;
use Extly\Illuminate\Contracts\Support\Responsable;
use Extly\Illuminate\Database\Eloquent\ModelNotFoundException;
use Extly\Illuminate\Database\MultipleRecordsFoundException;
use Extly\Illuminate\Database\RecordsNotFoundException;
...
use Extly\Illuminate\Http\Response;
use Extly\Illuminate\Routing\Router;
use Extly\Illuminate\Session\TokenMismatchException;
...
use Extly\Symfony\Component\HttpFoundation\RedirectResponse as SymfonyRedirectResponse;
use Extly\Symfony\Component\HttpFoundation\Response as SymfonyResponse;
use Extly\Symfony\Component\HttpKernel\Exception\AccessDeniedHttpException;
...
use Extly\Symfony\Component\HttpKernel\Exception\NotFoundHttpException;
use Throwable;
...
class Handler implements ExceptionHandlerContract
{
    ...
    public function reportable(callable $reportUsing)
    {
        ...
        return XT_tap(new ReportableHandler($reportUsing), function ($callback) {
    ...
    {
        return $request->expectsJson()
                    ? XT_response()->json(['message' => $exception->getMessage()], 401)
                    : XT_redirect()->guest($exception->redirectTo() ?? XT_route('login'));
    }
    ...
    {
        return XT_redirect($exception->redirectTo ?? XT_url()->previous())
    ...
    {
        if (! $this->isHttpException($e) && XT_config('app.debug')) {
    ...
    {
        try {
            return XT_config('app.debug') && class_exists(Whoops::class)
                        ? $this->renderExceptionWithWhoops($e)
                        : $this->renderExceptionWithSymfony($e, XT_config('app.debug'));
        } catch (Exception $e) {
            return $this->renderExceptionWithSymfony($e, XT_config('app.debug'));
        }
    }
    ...
    {
        return XT_tap(new Whoops, function ($whoops) {
    ...
    {
        try {
            return XT_app(HandlerInterface::class);
    ...
    protected function renderHttpException(HttpExceptionInterface $e)
    {
        ...
        if (XT_view()->exists($view = $this->getHttpExceptionView($e))) {
            return XT_response()->view($view, [
    ...
    {
        return XT_config('app.debug') ? [
            'message' => $e->getMessage(),
            'exception' => get_class($e),
            'file' => $e->getFile(),
            'line' => $e->getLine(),
            'trace' => XT_collect($e->getTrace())->map(function ($trace) {
