...
use Extly\Illuminate\Contracts\Translation\Translator;
use Extly\Illuminate\Contracts\Validation\DataAwareRule;
use Extly\Illuminate\Contracts\Validation\ImplicitRule;
use Extly\Illuminate\Contracts\Validation\Rule as RuleContract;
use Extly\Illuminate\Contracts\Validation\Validator as ValidatorContract;
use Extly\Illuminate\Contracts\Validation\ValidatorAwareRule;
use Extly\Illuminate\Support\Arr;
use Extly\Illuminate\Support\Fluent;
use Extly\Illuminate\Support\MessageBag;
use Extly\Illuminate\Support\Str;
use Extly\Illuminate\Support\ValidatedInput;
use InvalidArgumentException;
use RuntimeException;
use stdClass;
use Extly\Symfony\Component\HttpFoundation\File\UploadedFile;
...
class Validator implements ValidatorContract
{
    ...
    {
        XT_throw_if($this->fails(), $this->exception, $this);
    ...
    {
        XT_throw_if($this->invalid(), $this->exception, $this);
        ...
        foreach ($this->getRules() as $key => $rules) {
            ...
            $value = XT_data_get($this->getData(), $key, $missingValue);
    ...
    {
        return XT_collect($this->messages()->toArray())->map(function ($message, $key) {
    ...
    {
        $rules = XT_collect($rules)->mapWithKeys(function ($value, $key) {
    ...
    private function dataForSometimesIteration(string $attribute, $removeLastSegmentOfAttribute)
    {
        ...
        return is_array($data = XT_data_get($this->data, $attribute))
            ? new Fluent($data)
            : $data;
    }
