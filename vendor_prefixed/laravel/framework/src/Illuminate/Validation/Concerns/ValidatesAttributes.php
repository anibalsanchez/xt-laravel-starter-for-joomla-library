...
namespace Extly\Illuminate\Validation\Concerns;

use Countable;
use DateTime;
use DateTimeInterface;
use Extly\Egulias\EmailValidator\EmailValidator;
use Extly\Egulias\EmailValidator\Validation\DNSCheckValidation;
use Extly\Egulias\EmailValidator\Validation\MultipleValidationWithAnd;
use Extly\Egulias\EmailValidator\Validation\NoRFCWarningsValidation;
use Extly\Egulias\EmailValidator\Validation\RFCValidation;
use Extly\Egulias\EmailValidator\Validation\SpoofCheckValidation;
use Exception;
...
use Extly\Illuminate\Validation\ValidationData;
use InvalidArgumentException;
use Extly\Symfony\Component\HttpFoundation\File\File;
...
trait ValidatesAttributes
{
    ...
    public function validateEmail($attribute, $value, $parameters)
    {
        ...
        $validations = XT_collect($parameters)
            ->unique()
    ...
    public function guessColumnForQuery($attribute)
    {
        if (in_array($attribute, Arr::collapse($this->implicitAttributes))
                && ! is_numeric($last = XT_last(explode('.', $attribute)))) {
            return $last;
        }
