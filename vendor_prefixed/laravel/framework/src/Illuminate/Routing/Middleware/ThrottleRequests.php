...
use Closure;
use Extly\Illuminate\Cache\RateLimiter;
use Extly\Illuminate\Cache\RateLimiting\Unlimited;
use Extly\Illuminate\Http\Exceptions\HttpResponseException;
use Extly\Illuminate\Http\Exceptions\ThrottleRequestsException;
...
class ThrottleRequests
{
    ...
    protected function handleRequestUsingNamedLimiter($request, Closure $next, $limiterName, Closure $limiter)
    {
        ...
        return $this->handleRequest(
            $request,
            $next,
            XT_collect(Arr::wrap($limiterResponse))->map(function ($limit) use ($limiterName) {
                return (object) [
