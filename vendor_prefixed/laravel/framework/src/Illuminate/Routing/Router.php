...
use Extly\Illuminate\Contracts\Routing\Registrar as RegistrarContract;
...
use Extly\Illuminate\Routing\Events\RouteMatched;
...
class Router implements BindingRegistrar, RegistrarContract
{
    ...
    {
        return $this->any($uri, '\Extly\Illuminate\Routing\RedirectController')
    ...
    {
        return $this->match(['GET', 'HEAD'], $uri, '\Extly\Illuminate\Routing\ViewController')
                ->setDefaults([
                    'view' => $view,
                    'data' => $data,
    ...
    {
        $route = XT_tap($this->routes->getByName($name))->bind($this->currentRequest);
    ...
    {
        $excluded = XT_collect($route->excludedMiddleware())->map(function ($name) {
            return (array) MiddlewareNameResolver::resolve($name, $this->middleware, $this->middlewareGroups);
        })->flatten()->values()->all();

        $middleware = XT_collect($route->gatherMiddleware())->map(function ($name) {
            return (array) MiddlewareNameResolver::resolve($name, $this->middleware, $this->middlewareGroups);
            ...
            return XT_collect($excluded)->contains(function ($exclude) use ($reflection) {
