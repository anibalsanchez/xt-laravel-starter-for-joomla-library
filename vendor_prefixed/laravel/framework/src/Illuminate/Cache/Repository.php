...
use Extly\Illuminate\Cache\Events\CacheHit;
use Extly\Illuminate\Cache\Events\CacheMissed;
use Extly\Illuminate\Cache\Events\KeyForgotten;
use Extly\Illuminate\Cache\Events\KeyWritten;
use Extly\Illuminate\Contracts\Cache\Repository as CacheContract;
...
class Repository implements ArrayAccess, CacheContract
{
    ...
    {
        $values = $this->store->many(XT_collect($keys)->map(function ($value, $key) {
            return is_string($key) ? $key : $value;
        })->values()->all());

        return XT_collect($values)->map(function ($value, $key) use ($keys) {
    ...
    protected function handleManyResult($keys, $key, $value)
    {
        ...
        if (is_null($value)) {
            ...
            return isset($keys[$key]) ? XT_value($keys[$key]) : null;
        }
    ...
    {
        return XT_tap($this->get($key, $default), function () use ($key) {
            $this->forget($key);
        });
    }
    ...
    {
        return XT_tap($this->store->forget($this->itemKey($key)), function ($result) use ($key) {
            if ($result) {
