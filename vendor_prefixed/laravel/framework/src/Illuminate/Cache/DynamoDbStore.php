...
use Extly\Illuminate\Contracts\Cache\LockProvider;
...
class DynamoDbStore implements LockProvider, Store
{
    ...
    public function many(array $keys)
    {
        ...
        $response = $this->dynamo->batchGetItem([
            'RequestItems' => [
                $this->table => [
                    'ConsistentRead' => false,
                    'Keys' => XT_collect($prefixedKeys)->map(function ($key) {
                        return [
                            $this->keyAttribute => [
                                'S' => $key,
                            ],
        ...
        return array_merge(XT_collect(array_flip($keys))->map(function () {
            //
        })->all(), XT_collect($response['Responses'][$this->table])->mapWithKeys(function ($response) use ($now) {
    ...
    public function putMany(array $values, $seconds)
    {
        ...
        $this->dynamo->batchWriteItem([
            'RequestItems' => [
                $this->table => XT_collect($values)->map(function ($value, $key) use ($expiration) {
                    return [
                        'PutRequest' => [
                            'Item' => [
                                $this->keyAttribute => [
